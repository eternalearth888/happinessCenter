	...
	for($i = 1; $i <= $field_count; $i++) {
		...
		switch($field_type) {
			...
			case "comment":
				...
				$field = '<textarea' . $readonly.$disabled . ' cols="70" rows="8" name="comment" id="comment" class="' . $field_class . '"'. $onfocus.$fieldTitle.'>' . $field_value  . '</textarea>';
				  if ( $reg_exp<>'' )
			...
			case "textarea":
				...
				$field = '<textarea' . $readonly.$disabled . ' cols="70" rows="8" name="'.$input_name.'" id="'.$input_id.'" class="' . $field_class . '"'. $onfocus.$fieldTitle.'>' . $field_value  . '</textarea>';
				  if ( $reg_exp<>'' )
	           		 $field .= '<input type="hidden" name="'.$input_name.'_regexp" id="'.$input_id.'_regexp" value="'.$reg_exp.'"'.$fieldTitle.'/>';
				break;
